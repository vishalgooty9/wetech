...
if "sensor_data" not in st.session_state:
    st.session_state.sensor_data = generate_mock_readings()
...
with st.sidebar:
    ...
    st.caption(f"Last refresh: {st.session_state.last_refresh.strftime('%d %b %Y, %H:%M:%S')} IST")
...
if selected_area != "All areas":
    ...
    reports_view = [r for r in st.session_state.community_reports if r["location_name"] == selected_area]
else:
    df_view = df
    reports_view = st.session_state.community_reports
...
m4.metric("Community reports", len(st.session_state.community_reports))
    ...
    with form_col:
        ...
        with st.form("report_form", clear_on_submit=True):
            ...
            if submitted:
                ...
                st.success("Thanks! Your report has been added to the map and gallery.")

    with gallery_col:
        st.subheader("Photos reported by others")
        st.caption(f"{len(st.session_state.community_reports)} report(s) this session.")

        if not st.session_state.community_reports:
            st.info("No community reports yet. Be the first to submit one on the left.")
        else:
            for rep in st.session_state.community_reports:
